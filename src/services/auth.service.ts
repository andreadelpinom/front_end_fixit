...
class AuthService {
  ...
  async updateUser(userData: Partial<User>): Promise<AuthResponse> {
    ...
    const url = getApiUrl(`${API_CONFIG.ENDPOINTS.USERS.UPDATE}/${user.idUser}`);

    try {
      console.log('[AuthService] Updating user:', user.idUser);
      const response = extractData<AuthResponse>(
        await apiClient.put<unknown>(url, userData),
      );

      // Actualizar datos de usuario en storage
      if (response.user) {
        await storageService.saveUserData(response.user);
        console.log('[AuthService] User data updated successfully');
      }

      return response;
    } catch (error) {
      console.error('[AuthService] Update user failed:', error);
      ErrorUtils.logError(error, 'Update User');
      throw new Error(ErrorUtils.getErrorMessage(error));
    }
  }
