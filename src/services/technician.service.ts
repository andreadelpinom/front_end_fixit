...
export async function updateTechnicianProfile(
  ...
  const url = getApiUrl(`${API_CONFIG.ENDPOINTS.TECHNICIAN.UPDATE}/${idTecnico}`);
  return apiClient.put<Tecnico>(url, data);
}

// Actualizar estado del perfil técnico
export async function updateTechnicianStatus(
  idTecnico: number,
  status: string,
): Promise<Tecnico> {
  const url = getApiUrl(`${API_CONFIG.ENDPOINTS.TECHNICIAN.UPDATE}/${idTecnico}/status`);
  return apiClient.put<Tecnico>(url, { status });
}
