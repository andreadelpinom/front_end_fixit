...
class HomeService {
   ...
   */
  async getUsersStats(): Promise<{ totalUsers: number; totalTechnicians: number; totalClients: number }> {
    try {
      const url = getApiUrl('/admin/stats/users');
      const response = await apiClient.get<unknown>(url);
      const data = extractData<Record<string, unknown>>(response);
      return {
        totalUsers: Number(data?.totalUsers ?? 0),
        totalTechnicians: Number(data?.totalTechnicians ?? 0),
        totalClients: Number(data?.totalClients ?? 0),
      };
    } catch (error) {
      console.error('[HomeService] Error fetching users stats:', error);
      ErrorUtils.logError(error, 'HomeService.getUsersStats');
      throw error;
    }
  }

  /**
   * Obtiene estadísticas de solicitudes para admin
   * @returns Estadísticas de solicitudes
   *
   * Endpoint: GET /api/v1/admin/stats/requests
   */
  async getRequestsStats(): Promise<{ totalRequests: number; activeRequests: number }> {
    try {
      const url = getApiUrl('/admin/stats/requests');
      const response = await apiClient.get<unknown>(url);
      const data = extractData<Record<string, unknown>>(response);
      return {
      ...
      console.error('[HomeService] Error fetching requests stats:', error);
      ErrorUtils.logError(error, 'HomeService.getRequestsStats');
      throw error;
    }
  }
   ...
   *
   * Endpoint: GET /api/v1/admin/stats/notifications
   */
  async getNotificationsStats(): Promise<{ unreadCount: number }> {
    try {
      const url = getApiUrl('/admin/stats/notifications');
      const response = await apiClient.get<unknown>(url);
      const data = extractData<Record<string, unknown>>(response);
      return {
      ...
      console.error('[HomeService] Error fetching notifications stats:', error);
      ErrorUtils.logError(error, 'HomeService.getNotificationsStats');
      throw error;
    }
  }

  /**
   * Obtiene lista de usuarios para admin
  ...
  async getAllUsers(): Promise<User[]> {
    try {
      ...
      const response = await apiClient.get<unknown>(url);
      const data = extractArray<Record<string, unknown>>(response, ['users', 'data', 'items']);
      return data.map((item: any) => ({
        idUser: Number(item.idUser ?? item.id ?? 0),
        nombres: item.nombres ?? '',
        apellidos: item.apellidos ?? '',
        email: item.email ?? '',
        telefono: item.telefono ?? '',
        roles: Array.isArray(item.roles) ? item.roles.map((r: any) => r.toString()) : ['CLIENTE'],
        estado: item.estado ?? 'ACTIVO',
        createdAt: item.createdAt ?? new Date().toISOString(),
      }));
    } catch (error) {
      console.error('[HomeService] Error fetching all users:', error);
      ErrorUtils.logError(error, 'HomeService.getAllUsers');
      throw error;
    }
  }
